...
export default function (event, target, container, tooltipEl, place, isFollowMouse, offset) {
  ...
  if (place === 'top') {
    ...
    const yCursor = mouseY - (heightTooltipEl + actualOffset);
  ...
  if (place === 'bottom') {
    const xCursor = mouseX - (widthTooltipEl / 2);
    const yCursor = mouseY + actualOffsetReverse;
    const rightEl = xCursor + widthTooltipEl;
    ...
    if (bottomEl > bottomContainerEl) { // to top
  ...
  // ↓↓↓ right
  if (place === 'right') {
    const xCursor = mouseX + actualOffset;
  ...
  // ↓↓↓ left
  if (place === 'left') {
    const xCursor = mouseX - (widthTooltipEl + actualOffset);
    const yCursor = mouseY - (heightTooltipEl / 2);
    const rightEl = xCursor + widthTooltipEl;
    const bottomEl = yCursor + heightTooltipEl;
